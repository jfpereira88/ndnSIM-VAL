...
private:
     ...
     */
    void
    retransmissionTimerCallback(const std::shared_ptr<pft::Entry>& pftEntry, const nfd::FaceId outFaceId);

    void
    selfCleanTimerCallback(const std::shared_ptr<pft::Entry>& pftEntry);
...
private:
    ...
    uint32_t m_inValPkts;
    uint32_t m_outValPkts;
    uint32_t m_inDataPkts;
    uint32_t m_inIntPkts;
    uint32_t m_outDataPkts;
    uint32_t m_outIntPkts;
    uint32_t m_rtxDataPkts;
    uint32_t m_rtxIntPkts;
    uint32_t m_HLT_to_RTX;
    // faces
    std::shared_ptr<nfd::face::Face> m_commValFace;
};
