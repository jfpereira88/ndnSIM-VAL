 ...
 * ######################################
 */
public: //upper interface to be used by ValForwarder
  /** \ingroup val 
   *  \brief sends ValPacket on face
   *  \param valPacket a packet with ValHeader
   *  jfp 2019
   *  mieti - uminho
   */
  void
...
public: //static properties
  /** \ingroup val  
   * \brief to identify Val faces - just a boolean
   *  \return true if is Valface false otherwise
   *  jfp 2019
   *  mieti - uminho
   */
  bool
  ...
  /** \ingroup val  
   * \brief to identify Valfaces - just a boolean
   *  sets m_isValNetFace to true
   *  jfp 2019
   *  mieti - uminho
   */
  void
 ...
 * #############################
 */
public: // geoFace interface to be used by ValForwarder
  /** \ingroup val 
   * \brief geoFace method to bypass transport
   *  sends Interest from VAL to the NFD forwarder
   *  jfp 2019
   *  mieti - uminho
   * \param interest the interest to pass
   * \note it uses afterReceiveInterest signal
   */
  void
  ...
  /** \ingroup val  
   * \brief geoFace method to bypass transport
   *  sends Data from VAL to the NFD forwarder
   *  jfp 2019
   *  mieti - uminho
   *  \param data the data to pass
   *  \note it uses afterReceiveData signal
   */
  void
  sendDataToForwarder(const Data& data);

public: // geoFace interface to be used by nfd::Forwarder
  /** \ingroup val  
   * \brief geoFace method to bypass transport
   *  sends Interest from NFD forwarder to VAL
   *  jfp 2019
   *  mieti - uminho
   * \param interest the interest pkt to pass to VAL
   */
  void
  ...
  /** \ingroup val  
   * \brief geoFace method to bypass transport
   *  sends Data from NFD forwarder to VAL
   *  jfp 2019
   *  mieti - uminho
   * \param data the data pkt to pass to VAL
   * \param noncelist a pointer to a vector with all the
   * interest nonces that this data satisfies.
   */
  void
...
public: //static properties
  /** \ingroup val  
   * \brief to identify GeoFaces - just a boolean
   *  \return true if is GeoFace false otherwise
   *  jfp 2019
   *  mieti - uminho
   */
  bool
...
private:
  ...
  uint64_t m_metric;
  bool m_isValNetFace;  // valface identifyer
  bool m_isGeoFace;  // geoFace identifyer
};
...
inline bool
Face::isValNetFace() const
{
  return m_isValNetFace;
}

// must be set before adding to face-table
inline void
Face::setValNetFace()
{
  m_isValNetFace = true;
...
inline void
Face::sendValPacket(const ::ns3::ndn::val::ValPacket& valPacket)
{
  if(m_isValNetFace)
    m_service->sendValPacket(valPacket);
}
...
inline void
Face::sendInterestToForwarder(const Interest& interest)
{
  if(m_isGeoFace)
    m_service->sendInterestToForwarder(interest);
}

inline void
Face::sendDataToForwarder(const Data& data)
{
  if(m_isGeoFace)
    m_service->sendDataToForwarder(data);
}

inline void
Face::sendInterestToVal(const Interest& interest)
{
  if(m_isGeoFace)
    m_service->sendInterestToVal(interest);
}

inline void
Face::sendDataToVal(const Data& data, std::vector<uint32_t> *nonceList, bool isProducer)
{
  if(m_isGeoFace)
    m_service->sendDataToVal(data, nonceList, isProducer);
}
