...
FaceTable::addImpl(shared_ptr<Face> face, FaceId faceId)
{
  ...
  // #########  new signals for VAL support
  // jfp-uminho-mieti 2019
  if(face->isValNetFace())
    this->afterValFaceAdd(*face);
  else
    this->afterAdd(*face);
}
...
FaceTable::remove(FaceId faceId)
{
  ...
  // jfp-uminho-mieti 2019
  if(face->isValNetFace())
    this->beforeValFaceRemove(*face); // received on val::ValForwarder
  else
    this->beforeRemove(*face); // received on nfd::Forwarder
