...
/**
   ...
   */
  void
  Strategy::sendDataToVal(const shared_ptr<pit::Entry>& pitEntry, const Data& data, const Face& outFace, std::vector<uint32_t> *nonceList, bool isProducer)
  {
    BOOST_ASSERT(pitEntry->getInterest().matchesData(data));
    pitEntry->deleteInRecord(outFace);
    m_forwarder.onOutgoingData(data, *const_pointer_cast<Face>(outFace.shared_from_this()), nonceList, isProducer);
  }
