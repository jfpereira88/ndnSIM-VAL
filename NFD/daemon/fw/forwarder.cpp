  ...
  , m_csFace(face::makeNullFace(FaceUri("contentstore://")))
{
  ...
  // TODO: needs to identify it is using LAL or not
  ...
  if (nextHopTag != nullptr) {
    ...
    if (nextHopFace != nullptr) {
      ...
      if(inFace.isGeoFace()) { 
        ...
        const fib::Entry& fibEntryRef = m_fib.findLongestPrefixMatch(*pitEntry);
        if(fibEntryRef.hasNextHops()) {
          const fib::NextHopList& nexthops = fibEntryRef.getNextHops();
          for (const auto& nexthop : nexthops) {
            Face& outFace = nexthop.getFace();
            if(outFace.getLocalUri().toString() == "appFace://") { // is appFace
              // remove forwarding Hint
              const_cast<Interest&>(interest).setForwardingHint({});
              nextHopFace = &outFace;  // send to appFace;
              break;
            }
          }
        } else {  // relay node
          // here we create a fibentry:
          // if it has a forwardingHint the Interest knows its own destination area
          /* @TODO: some control must be done in order to remove this entry in case that no data
            arrives to satisfy this Interest */
          fib::Entry* fibEntry = m_fib.insert(interest.getName().getPrefix(-1)).first;
          fibEntry->addOrUpdateNextHop(*nextHopFace, 0, 1);
        }
      }
      if(inFace.getLocalUri().toString() == "appFace://") {  // consumer node
        fib::Entry* fibEntry = m_fib.insert(interest.getName().getPrefix(-1)).first;
        fibEntry->addOrUpdateNextHop(*nextHopFace, 0, 1);
      }
